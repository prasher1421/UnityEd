...
class after_10th_intermediate : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view= inflater.inflate(R.layout.fragment_after_10th_intermediate, container, false)
        val course_name:ArrayList<String> =ArrayList<String>()
        val course_map:HashMap<String,ArrayList<String>> = HashMap()

        try{
//            var i:Int=0
            for(i in 0 until after_10th_Model.data[0].streams[0].course.size) {
                course_name.add(after_10th_Model.data[0].streams[0].course[i].name)
                val sub_course:ArrayList<String> = after_10th_Model.data[0].streams[0].course[i].sub_course
        ...
        text_int.text=after_10th_Model.data[0].streams[0].duration

        listadapter= Course_Type_Adapter(activity!!, course_name, course_map)

        view.findViewById<ExpandableListView>(R.id.elist_10th_int).setAdapter(listadapter)

        val duration_view=view.findViewById<LinearLayout>(R.id.duration_linear_after10th)
        if(after_10th_Model.data[0].streams[0].duration.length==0){
