...
class after_intermediate_teach : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_after_intermediate_teach, container, false)
        val degree_name:ArrayList<String> =ArrayList<String>()
        val deg_cour_map:HashMap<String,ArrayList<String>> = HashMap()

        try{
//            var i:Int=0
            for(i in 0 until after_Intermediate_Model.data[0].streams[4].course.size) {
                degree_name.add(after_Intermediate_Model.data[0].streams[4].course[i].degree_name +
                 "   " + after_Intermediate_Model.data[0].streams[4].course[i].course_duration)
                val sub_course:ArrayList<String> = after_Intermediate_Model.data[0].streams[4].course[i].sub_course
...
//        view.findViewById<TextView>(R.id.duration).text="Duration: "+after_Intermediate_Model.data[0].streams[4].duration
        ...
        if (after_Intermediate_Model.data[0].streams[4].eligible.length==0){
    ...
    companion object {
        ...
        fun newInstance(param1: String, param2: String) =
            after_intermediate_teach().apply {
